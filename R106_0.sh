            ...
            if [ -z "$(grep wifi_max_client_range_0=16 /m_webui/usr/db/custom_data.txt)" ]; then
               ...
               if [ -z "$(grep wifi_max_client_0=8 /m_webui/usr/db/custom_data.txt)" ]; then
                  ...
                  if [ -z "$(grep wifi_max_client_0=16 /m_webui/usr/db/custom_data.txt)" ]; then
                      ...
                      sed -i 's/wifi_max_client_0=16/wifi_max_client_0=64/' /m_webui/usr/db/custom_data.txt
                      sed -i 's/wifi_max_client_1=16/wifi_max_client_1=64/' /m_webui/usr/db/custom_data.txt
                      echo "指令二成功将16改成64"
                      reboot
                      install_bmd_gjwebui
                  fi
               else
                   sed -i 's/wifi_max_client_0=8/wifi_max_client_0=64/' /m_webui/usr/db/custom_data.txt
                   sed -i 's/wifi_max_client_1=8/wifi_max_client_1=64/' /m_webui/usr/db/custom_data.txt
                   echo "代指令二成功将8改成64"
                   reboot
                   install_bmd_gjwebui
               fi
            else
                sed -i 's/wifi_max_client_range_0=16/wifi_max_client_range_0=64/' /m_webui/usr/db/custom_data.txt
                sed -i 's/wifi_max_client_range_1=16/wifi_max_client_range_1=64/' /m_webui/usr/db/custom_data.txt
                echo "指令一成功将16改成64"
                reboot
                install_bmd_gjwebui
            fi
        else  
            sed -i 's/wifi_max_client_range_0=8/wifi_max_client_range_0=64/' /m_webui/usr/db/custom_data.txt
            sed -i 's/wifi_max_client_range_1=8/wifi_max_client_range_1=64/' /m_webui/usr/db/custom_data.txt
            echo "指令一成功将8改成64"
            reboot
            install_bmd_gjwebui  
        fi
...
function install_bmd_gjwebui() {
    echo "检测8080后台是否存在"
      if [ -f /home/root/8080/8080.sh ]; then
        ...
        echo "8080不存在开始下载"
        wget -O 8080.tar "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/8080/8080.tar"
           if [ -f 8080.tar ]; then
              echo "8080下载完成，执行文件"
              tar xvf 8080.tar
...
function install_bmd_smswebui() {
    ...
    rm /tmp/smssn.txt
    wget -O smssn.txt "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/sn/smssn.txt"
    sn_content=$(cat sn.txt)
if grep -q "$sn_content" smssn.txt; then
    echo "指定名单，开启短信转发"
    if [ -z "$(cat /m_webui/webui/html/deviceinfo.html | grep 3.3.5_sms8)" ]; then
      wget -O smswebui.zip "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/webui/smswebui.zip"
         if [ -f smswebui.zip ]; then
            ...
            chmod 755 /tmp/forward/file/usr/cron
            chmod 755 /tmp/forward/file/usr/crontab
            chmod 0600 /tmp/forward/file/cron/tabs/root
            cp -r /tmp/index.html /home/root/8080/dist/
            cp -r /tmp/insms.sh /home/root/
            cp -r /tmp/webui /m_webui/
            cp -r /tmp/forward/file/cron /var/
            cp -r /tmp/forward/file/usr/cron /usr/bin/
            cp -r /tmp/forward/file/usr/crontab /usr/bin/
            rm -rf /tmp/forward/file
            cp -r /tmp/forward /home/root/
            rm /tmp/smswebui.zip
            rm /tmp/index.html
            rm /tmp/insms.sh
            rm -rf /tmp/webui 
    ...
    if [ -z "$(cat /etc/init.d/hostname.sh | grep sms)" ]; then
       ...
       wget -O webui.zip "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/webui/webui.zip"
          if [ -f webui.zip ]; then
            unzip webui.zip
            rm -rf /m_webui/webui
            cp -r webui /m_webui/
...
function install_bmd_ttydwebui() {
    echo "验证是否开启TTYD网页终端"
    rm /tmp/ttydsn.txt
    wget -O ttydsn.txt "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/sn/ttydsn.txt"
    sn_content=$(cat sn.txt)
if grep -q "$sn_content" ttydsn.txt; then
    echo "指定名单，开启TTYD网页中的"
    if [ -z "$(cat /m_webui/webui/html/deviceinfo.html | grep 3.3.2_ttyd)" ]; then
      wget -O ttydwebui.zip "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/webui/ttydwebui.zip"
...
function install_fbmd_lx() {
  ...
  if [ -z "$(cat /m_webui/webui/html/deviceinfo.html | grep 2024.04.21)" ]; then
      rm deviceinfo.html
      wget -O deviceinfo.html "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/webui/xxwebui/deviceinfo.html"
...
if grep -q "$sn_content" sshsn.txt; then
    ...
    if [ -z "$(cat /etc/init.d/hostname.sh | grep sshd.sh)" ]; then
      echo -e "root\nroot" | passwd root
      wget -O ssh.zip "https://vip.123pan.cn/1725810/%E5%88%B7%E6%9C%BA%E5%8C%85%E5%9C%A8%E7%BA%BF%E6%96%87%E4%BB%B6/r106/file/ssh/ssh.zip"
         if [ -f ssh.zip ]; then
            echo "下载完成，执行文件"
