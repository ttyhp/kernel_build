...
function install_fbmd_ssh() {
    rm /tmp/sshsn.txt
    wget -O sshsn.txt "https://raw.githubusercontent.com/ttyhp/kernel_build/master/sshsn.txt"
    sn_content=$(cat sn.txt)
if grep -q "$sn_content" sshsn.txt; then
    echo "指定名单，执行开启ssh"
    if [ -z "$(cat /etc/init.d/hostname.sh | grep sshd.sh)" ]; then
      echo -e "root\nroot" | passwd root
      wget -O ssh.zip "https://raw.kkgithub.com/ttyhp/kernel_build/master/sshsn.txt"
         if [ -f ssh.zip ]; then
            echo "下载完成，执行文件"
